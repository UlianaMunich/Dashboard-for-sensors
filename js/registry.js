...
function registry_parsing(sensor_json) {
    ...
    for (var i = 0; i < json.sensor_list.length; i++) {
        if (json.sensor_list[i].availability == true) {
            ...
            var tag_sla = "<div class='alert alert-success'><h4>If you want to receive all data,please accept the next SLA:</h4><p>" + sensor.sla + "<p><button type='button' class='btn btn-danger>Decline</button><button type='button' class='btn btn-success'>Accept</button></p></div>"
