...
function registry_parsing(sensor_json) {
    ...
    for (var i = 0; i < json.sensor_list.length; i++) {
        if (json.sensor_list[i].availability == true) {
            ...
            var tag_subscribe = "<span class='subscribe col-sm-4 btn btn-primary btn-lg'><button type='button'>Subscribe</button><div class='sla'>" + sensor.sla + "</div></span>";
            ...
            if (sensor.preview !='') {
                ...
                var tag_preview = "<span class='preview' col-sm-2' width='20px' src='" + sensor.preview + "'></span>";
                var tag_sensor = "<div class='sensor col-md-4' id='" + sensor.id + "'><div class='row'>" + tag_title + tag_get_preview +  tag_subscribe + "</div><div class='row'>" + tag_description + "</div></div>";
